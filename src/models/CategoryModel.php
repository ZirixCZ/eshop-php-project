...
public function fetchAllCategories() {
    ...
    public function fetchAllCategoriesWithChildren($pdo, $parentId = null, $breadcrumbs = []) {
    $categories = $this->fetchCategories($parentId);
    foreach ($categories as $key => $category) {
        $newBreadcrumbs = array_merge($breadcrumbs, [$category]);
        $childCategories = $this->fetchAllCategoriesWithChildren($pdo, $category['id'], $newBreadcrumbs);
